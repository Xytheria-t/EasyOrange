...
> 0000 为模板；编号篇数见[结构计数](../工程指标.md#结构计数)。**推荐阅读顺序**：先读「现役决策」（按主题：ID 与数据建模 → 事务一致性 → 模块边界 → 消息选型 → CQRS → AI → DDD 治理），再读「演进方案」（不实施，触发条件未到），最后按需翻阅[已替代决策.md](已替代决策.md)（决策反转链的压缩记录）。
...
| [0008](0008-ai-spring-ai-framework.md) | AI 框架化 | 接受 | 全面框架化 Spring AI，删除自研 Port/Adapter/指标（**Supersedes ADR-0003**） | 2026-08-03 |
...
三份已被替代的记录压缩归档为一份（只留「当时决策 / 拒绝项 / 今天仍成立的结论」）：[已替代决策.md](已替代决策.md)。其中 **ADR 0004 的 `@TokenBudget` 日预算仍现役**（Bulkhead 部分已删）。
