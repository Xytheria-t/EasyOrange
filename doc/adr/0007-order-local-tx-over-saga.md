...
1. **原子性本就不需要 Saga**：`OrderCommandHandler.createOrder(CreateOrderCommand)` 的 `TransactionTemplate` 事务边界使订单、库存扣减（`ProductInventoryPort.decreaseStock`，REQUIRED 传播）与支付创建（`PaymentGatewayPort.createPayment`，REQUIRED 传播）全部进入**同一 MySQL 事务**，任一步失败数据库整体回滚，无需任何编排/补偿。
...
约束未变：单数据库部署（MySQL）、平台不经手资金、C2C 直发、模块边界用 Port/Adapter + `<optional>` 隔离。

## 决策（Decision）

**移除订单创建的 Saga 层**，回归最简可靠形态：

1. **原子性 = 本地单事务**。`OrderCommandHandler.createOrder(CreateOrderCommand)` 在 `TransactionTemplate` 显式控制的单一本地事务内依次执行：**先取分布式锁（锁等待在事务外，不占数据库连接）** → 事务内准备商品 → 创建订单 + 发布 `OrderCreatedEvent`（Outbox 同事务原子）→ `decreaseStock` → `createPayment`。任一步失败事务整体回滚，抛 `OrderDomainException`（B3009），**无补偿路径**。
