...
**检索改为两路独立召回（kNN / BM25 各查一次）后在索引侧做 RRF 排名融合；删除 Java 侧余弦重排，命中不再回传分块向量；同时把语料扩到 25 篇（含 20 篇同域干扰文档）让 hit@5 恢复判别力。**
...
4. **语料扩到 25 篇**：新增 kb-0006~kb-0025 共 20 篇**同域干扰文档**（也是真实平台规则，非随机噪声），使 `topK=5` 面对 25 个分块成为真正的选择问题；金标准集相应拆分为 20 条 `scope: chat` + 15 条 `scope: retrieval`（新增 5 条覆盖干扰文档），并由 `GoldenSetLoader` 在加载期强校验 scope 与字段自洽性。
...
- **旧口径指标作废**：`doc/工程指标.md` 里 2026-09-17 的 `hit@5 100% / MRR 0.95` 是 5 篇语料下测的，扩语料后必须重跑才能引用；本次改动未在本地实测（本地无 AI key），由每周 `ai-eval.yml` 回归给出首个可信数字。
...
- 决策回顾周期：下次 ai-eval 回归给出新口径数字后回看（每周一 03:30 自动执行）
