...
- **9 个独立事件消费者**（每个模块独占一个队列 `eo.{name}`，队列数 = 消费者数 = DLQ 队列数），完全是 pub/sub 模式：**同一条领域事件被多个下游各消费各的、各 ack 各的、各有各的失败重试链路**
...
- **切换成本必须可接受**（不接受花 3-5 天重写 9 消费者 + 重试链路 + 测试）
...
2. **消息模型贴合度（权重 30%）**：EasyOrange 是「1 事件 → 9 独立下游消费」——RabbitMQ 的 Exchange → 多独立队列模型是原生的，每个下游独立 ack/nack/DLQ/重试互不干扰；Kafka 用 9 个不同 consumer group 硬做是可行的，但「每个 consumer group 一个 offset + 无原生 DLQ」会把重试链路拆成 9 份分散逻辑，讲解与调试都更难；NATS JetStream 的 Pull Consumer 独立位点模型匹配度较高；Pulsar Shared subscription 也 OK。
3. **替换工程成本（权重 20%）**：RabbitMQ 是 0 成本（已落地，9 消费者 + DLQ 调度器 + 幂等 + traceId 全链路稳定运行）；NATS JetStream 备选 Adapter 估算 1-2 天（官方 `nats-spring-boot-starter` 成熟）；Kafka 估算 3-5 天（@RabbitListener 重写 + DLQ 自实现 + Spring Modulith Kafka 对接 + 测试 mock 全换）；Pulsar 估算 4-6 天（无成熟官方 starter + 本地运维重）。
...
- 无切换风险：现有 9 消费者 + Outbox + DLQ 三级重试 + traceId 全链路完全保留，测试不需要重写
...
- **NATS JetStream 2.10+**：**未默认采用，但保留作为首选备选 Adapter**。优点：① 单进程超轻（`nats:2.10 -js`，50MB 镜像）；② 原生 `max_deliver` + discard dead letter policy（接近 Rabbit 的 DLQ 能力）；③ 消费模型（Subject 路由 + Pull Consumer 独立位点）完全匹配 9 独立消费者场景。缺点：① 官方 `nats-spring-boot-starter` 的注解式消费（`@NatsListener` / `@JetStreamListener`）成熟度不如 `@RabbitListener`，ACK 语义需要手动控制（Ack/Nak/Term）；② 大众认知度不如 Kafka，容易被追问「为什么用 NATS 不用 Kafka」——正好用本 ADR 回应。保留作为 `@ConditionalOnProperty` 可切换的备选实现，1-2 天可落地。
...
- 相关代码：
  - [RabbitMQConfig.java](../../easyorange-backend/easyorange-framework/src/main/java/com/cartethyia/easyorange/framework/messaging/config/RabbitMQConfig.java)：交换机 + 9 队列 + DLQ 绑定声明
