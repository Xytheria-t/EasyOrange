...
| 时间精度 | 业务表 / 观测表 DATETIME，Spring Modulith 事件表 TIMESTAMP(6) |
| 外键 | 无物理外键，通过应用层保证一致性 |
| 全文索引 | 主检索由 Elasticsearch + IK 分词器承担（`infra/elasticsearch` 镜像内置 analysis-ik，版本硬锁见该 Dockerfile 注释，经 `docker compose --profile search` 启用）；另建两处 ngram FULLTEXT（`ft_eo_product_name` / `ft_eo_product_search_text`）作 ES 关闭时的 MySQL 降级检索，`search_text` 由索引适配器与 ES 双写维护 |
...
**Flyway 配置要点**：`clean-disabled: true`（生产禁止 clean）；`validate-on-migrate: true` 为干净库默认，dev / it profile 关闭——开发阶段改 V1 后 checksum 会挡启动，不必每次重置，想刷新 schema 时跑 [`scripts/db-reset.sh`](../scripts/db-reset.sh)（DROP/CREATE dev 与 IT 库一体）重跑应用即从头执行 V1。

**常用命令**：`mvn flyway:info`（状态）/ `flyway:migrate` / `flyway:validate`（手动校验，需干净历史）/ `flyway:repair`（修 checksum，谨慎）/ `flyway:clean`（仅开发）。
...
V1 单脚本建全部表（`eo_*` 业务/观测表 + 2 个 Spring Modulith 基础设施表）——**总数以[结构计数](./工程指标.md#结构计数)为准**。
...
| 商品 | eo_product_audit_log | 商品审核记录 | ProductAuditLogDO |
...
eo_user ──1:N── eo_product (user_id)
              ...
              ├──1:N── eo_stock_ledger (product_id)
              └──1:N── eo_product_audit_log (product_id)
...
eo_user ──1:N── eo_user_preference (user_id)

eo_ai_call_log ──1:N── eo_ai_feedback (call_log_id，可空)
```
