...
- 开发环境短信是内存 Mock，**不依赖 Redis、不发真实短信**，控制台打 `[MOCK SMS]`；`MockSmsCodeAdapter` 基于 `ConcurrentHashMap`（**重启即重置**），与 `RedisSmsCodeAdapter` 按 `@Profile` **互斥**（dev/test ↔ it/prod，装配由 `SmsCodeAdapterWiringTest` 守护；未声明 profile 的环境不注册任何实现、启动即失败，防裸跑上生产）；`MockSmsSenderAdapter` 用 `@Profile({"dev","test","default","it"})`，**生产不注册**
...
- **历史按原始角色传多消息，不压进当前 user 消息**：`ChatPromptAssembler.assemble`（纯装配类，与 `AiChatService` 的「缓存 / 记忆 / 降级 / 生成」编排分开）组装成 `[system, 历史 user/assistant …, 当前 user]`——跨轮次前缀稳定才能命中供应商的上下文缓存折扣，把历史塞进单条 user 消息会让每轮前缀都变、缓存全失效。与 `ChatContextTrimmer` 的裁剪是两件事：裁剪决定留多少，这里决定以什么形状传
