...
单点约定（工单-易变数字治理）：
  ...
  3. **反向检查**：计数出现在该区块之外即报错并指出 `文件:行` —— **无例外**（`modules` 亦不例外：
     ...
     （CQRS 作用域）、`V1 的 2 张 Modulith 表`这类子集口径放过（见 `TOLERANT_PATTERNS` / `CN_VOCAB`）。
...
#   - 裸写「N 张表」：V1 单脚本内的子集表数（如「V1 的 2 张事件表」）不等于总数
...
# 同形异义（`4 模块` CQRS 作用域、V1 单脚本子集表数、`11 个 DLQ` 决策时点口径）改由
